...
public class Game {
	...
	public Game(String playerName, String playerCity) throws IOException {
		...
		if (playerCity.toLowerCase().equalsIgnoreCase("cairo")) {
			loadArmy("Rome", "rome_army.csv");
			loadArmy("Sparta", "sparta_army.csv");
		} else if (playerCity.toLowerCase().equalsIgnoreCase("rome")) {
	...
	public void loadArmy(String cityName, String path) throws IOException {
		...
		for (City c : availableCities) {
			if (c.getName().toLowerCase().equalsIgnoreCase(cityName.toLowerCase())) {
	...
	public void targetCity(Army army, String targetName) {
		...
		if (army.getCurrentLocation().equalsIgnoreCase("onRoad")) {
			from = army.getTarget();
		}
		for (Distance d : distances) {
			if ((d.getFrom().equalsIgnoreCase(from) || d.getFrom().equalsIgnoreCase(targetName)) && (d.getTo().equalsIgnoreCase(from) || d.getTo().equalsIgnoreCase(targetName))) {
	...
	public void endTurn() {
		...
		for (Army a : player.getControlledArmies()) {
			if (!a.getTarget().equalsIgnoreCase("") && a.getCurrentStatus() == Status.IDLE) {
				a.setCurrentStatus(Status.MARCHING);
				a.setCurrentLocation("onRoad");
			}
			if (a.getDistancetoTarget() > 0 && !a.getTarget().equalsIgnoreCase("")) {
